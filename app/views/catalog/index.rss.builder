...
xml.rss(version: "2.0") {
  xml.channel {
    ...
    xml.language('en-us')
    @response.documents.each_with_index do |document, document_index|
      xml << render_xml_partials(document, blacklight_config.view_config(:rss).partials, document_counter: document_index + 1)
    end
  }
}
